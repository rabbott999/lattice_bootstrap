...
PARAMS+=" --kernel-type tau_moment"
if [[ -v SCALAR ]]; then
    if [[ -v ADD_NOISE ]]; then
	echo "Running noisy scalar case"
	PARAMS+=" --add-noise"
	PARAMS+=" --output ${DATA}/tau_moment_scalar_Nt${NT}_noisy.pickle"
    else
	echo "Running exact scalar case"
	PARAMS+=" --output ${DATA}/tau_moment_scalar_Nt${NT}.pickle"
    fi
    PARAMS+=" --scalar"
else
    if [[ -v ADD_NOISE ]]; then
	...
	PARAMS+=" --add-noise"
	PARAMS+=" --output ${DATA}/tau_moment_Nt${NT}_noisy.pickle"
    else
	echo "Running exact block case"
	PARAMS+=" --output ${DATA}/tau_moment_Nt${NT}.pickle"
    fi
fi
